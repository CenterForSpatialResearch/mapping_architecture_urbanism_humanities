...
![Layer Projection Metadata](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_01_NAD83_Projections.png)
...
* We will be using [FIPS region codes](https://en.wikipedia.org/wiki/List_of_FIPS_region_codes_(S%E2%80%93U)#US:_United_States) to join the Natural Earth vector boundaries with the Census population data. The value in the `ID` column is a concatenated string that combines the Census table ID with individual state FIPS codes. We need to separate out the FIPS code portion of this ID so that this dataset can match up with the Natural Earth dataset, which already uses FIPS codes to identify each state. To do that, we need to create a new column that pulls *only the last four characters* in the `ID` column. We will use the `Right` function in Excel to do this.
	...
	* In the first cell of the column, type the formula `=RIGHT(A2,4)`. This will pull the last four characters in the `ID` column, `A4`, into the new `FIPS_ID` column.
	...
	![Excel FIPS Formula](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_08_Excel_FIPS_Formula.png)
	...
	* After typing the formula, hit `Enter`. Double-click on the bottom right corner of the cell to populate the entire column with the new formula.
	...
	![Excel Populated FIPS Column](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_09_Excel_Populated_FIPS_Column.png)
	
	* Name the file `StatePopulations.csv`, and save it to the a `Windows Comma Separated` format.

	![Excel Save New CSV](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_10_Excel_Save_New_CSV.png)
...
* As in Tutorial 03, we need to create a file that describes the data types in this new CSV before attempting to import it into QGIS. We will therefore use a text editor to create a `.csvt` file.
	...
	* Save the file with the same name as the CSV file, only with the `.csvt` extension: `StatePopulations.csvt`. It should look something like this:
	...
	![Create CSVT file](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_11_Population_CSVT.png)
...
![Layer Projection Metadata](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_12_Layer_Projection_Metadata.png)
...
![Select U.S. States](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_13_Select_US.png)

* Hit `Select` and close the query builder. Navigating back to the map, only the U.S. should be selected.

![Selected U.S. States](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_14_Selected_US.png)
...
![Set CRS to Albers Projection](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_15_CRS_Albers.png)
...
![Check U.S. States Metadata](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_16_Check_US_Metadata.png)
...
![U.S. States Albers Projection](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_17_Check_US_Albers.png)
...
![Edit FIPS Alt Value for Minnesota](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_19_Editing_MN_FIPS.png)
...
![Import Populations CSV](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_22_Import_CSV.png)
...
![Join CSV to the U.S. Albers Layer](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_23_Join_CSV_Albers.png)
...
![Calculate New Population Density Field](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_24_New_Column_Density.png)
...
![Filter Out Washington D.C.](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_25_DC_Filter.png)
...
![Population Map](https://github.com/brianhouse/mapping-architecture-urbanism-humanities/blob/master/Images/06_26_Population_Map.png)
...
Tutorial created by Emily Fuhrman for the [Mapping for Architecture, Urbanism and the Humanities](https://github.com/juanfrans-courses/mapping_arch_hum) class at Columbia University. Edited by Brian House for [Fall 2018](https://github.com/brianhouse/mapping-architecture-urbanism-humanities).
